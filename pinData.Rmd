...
Make a WQM_Stations_Spatial dataset that has spatially derived ecoregion, subbasin, and assessment info.
...
WQM_Stations_SpatialEco <- #filter(WQM_Stations_Spatial1, is.na(US_L3CODE)) %>%
  st_intersection(WQM_Stations_Spatial1, dplyr::select(ecoregionLarge, US_L3CODE, US_L3NAME)) %>%st_drop_geometry()
#  mutate(US_L3CODE = as.character(`US_L3CODE.1`), US_L3NAME = as.character(`US_L3NAME.1`)) %>%
#  dplyr::select(-c(US_L3CODE.1, US_L3NAME.1)) %>% st_drop_geometry()

# Add Assessment and Basin Info
WQM_Stations_SpatialAssess <- #filter(WQM_Stations_Spatial1, is.na(ASSESS_REG)) %>%
  st_intersection(WQM_Stations_Spatial1, unrestrictedAssessmentRegionVAHU6Subbasin) %>% #assessmentRegionLarge) %>%
...
WQM_Stations_Spatial2 <- full_join(dplyr::select(WQM_Stations_SpatialEco, StationID:US_L3NAME),
                                        dplyr::select(WQM_Stations_SpatialAssess, StationID, ASSESS_REG:Basin_Code ),
                                        by='StationID')


WQM_Stations_Spatial <- bind_rows(WQM_Stations_Spatial_pin, WQM_Stations_Spatial2)
...
benSampsStationsEco <- filter(benSampsStations1, is.na(US_L3CODE)) %>%
  st_intersection(dplyr::select(ecoregionLarge, US_L3CODE, US_L3NAME)) %>%
...
benSampsStationsTogether <- left_join(benSampsStations1, benSampsStationsEco, 
  ...
  mutate(US_L3CODE = ifelse(is.na(US_L3CODE), US_L3CODE1, US_L3CODE),
         US_L3NAME = ifelse(is.na(US_L3NAME), US_L3NAME1, US_L3NAME)) %>%
  dplyr::select( StationID, Sta_Desc, `Total Station Visits (Not Sample Reps)`, US_L3CODE, US_L3NAME)
